...
export const MarkdownText = memo(({ text, className = "" }: MarkdownTextProps) => {
    ...
    const sanitizedHTML = useMemo(() => {
        ...
        return DOMPurify.sanitize(rendered, { ...DOMPURIFY_CONFIG, RETURN_DOM_FRAGMENT: false, RETURN_DOM: false });
    }, [text]);


    return <div className={`leading-relaxed ${className}`} dangerouslySetInnerHTML={{ __html: sanitizedHTML as string }} />;
});
