...
export const PaperLibrary = ({ onSelectPaper, onResumePaper, checkSessionForPaper }) => {
    ...
    // Refs to hold latest callbacks from props to prevent re-renders of memoized children
    ...
    const handleSelect = useCallback(async (paper) => {
        const onSelect = onSelectPaperRef.current;
        if (!onSelect) return;
    ...
    const handleResume = useCallback((paper) => {
        if (onResumePaperRef.current) {
